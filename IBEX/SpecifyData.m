...
if isempty(TempH)
    ImcontrastGUIFOA([handles.AxesImageAxial, handles.AxesImageSag, handles.AxesImageCor], [TempPos(1)+(TempPos(3)-300)/2,  TempPos(2)+TempPos(4)-100, 300, 125]);
end
    ...
    for k=1:PlanLen
        ...
        if ~isempty(SelectIndex)
            for j=1:length(SelectIndex)
                ...
                CDataSetInfo=GetDateSetROIInfo(CDataSetInfo, handles, BWMatIndex);  
                CDataSetInfo=GetStructAxialROI(CDataSetInfo, RowIndex, ColIndex, handles, UserTable);
                
                DataSetsInfo=[DataSetsInfo; CDataSetInfo];
