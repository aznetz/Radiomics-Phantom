function SegMask=AutoSegPET(ImageData, BoxMask, Para)
...
SUVMajor=PercentileArea(ImageData(BoxMaskT), 0.97);
...
for SliceNum = 1:size(ImageDataSub, 3);
    ...
    SUVMaxMajor=PercentileArea(I(BoxMaskSub), 0.98);
    SUVMaxMajor2=PercentileArea(I(BoxMaskSub), 0.9);
    
    I=I.*BoxMaskSub;    
    SUVmax=max(I(:));
    
    %Don't care no enough activity image
    if SUVMaxMajor < SUVMajor
        SegMask(:, :, SliceNum)=zeros(size(I), 'uint8');
        continue;
    end
    ...
    for i = 1:length(levels)
        ...
        TempIndex=find(I > levels(i)*SUVMaxMajor);   
    ...
    gradient_perim=gradient_perim.*(SUVperimmean<0.6*SUVMaxMajor2);%%%avoids fitting high gradient near peaks in tumor
    ...
    if SUVmax > 0
        ROI(I>levels(MaxGradIndex)*SUVMaxMajor) = 1;
