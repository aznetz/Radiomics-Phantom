function out = ZoomCommon_IFOA(varargin)
...
%   $Revision: 1.1.6.2 $  $Date: 2004/08/16 01:45:04 $
...
%---Return when on status of dragging box
Handles = guihandles(gcbo);
GuiHandles=guidata(Handles.figure1);

CurrentAxisBack = get(gcf,'CurrentAxes');

if ~isfield(GuiHandles, 'AxesImageAxial') || ~isfield(GuiHandles, 'AxesImageCor') || ~isfield(GuiHandles, 'AxesImageSag') || ...
        (CurrentAxisBack ~= GuiHandles.AxesImageAxial && CurrentAxisBack ~= GuiHandles.AxesImageCor &&CurrentAxisBack ~= GuiHandles.AxesImageSag)
...
if strcmp(zoomCommand,'toggle'),
    state = getappdata(fig,'ZOOMFigureState');
    if isempty(state)
        ZoomCommon_IFOA(fig,'on');
    else
        ZoomCommon_IFOA(fig,'off');
    end
    % done, go home
    return
end % if
...
case 'down'
    ...
    limits = ZoomCommon_IFOA(fig,'getlimits');
...
case 'on',
    ...
    if isempty(state),
        ...
        state = uiclearmode(fig,'docontext','ZoomCommon_IFOA',fig,'off');
    ...
    set(fig,'windowbuttondownfcn','ZoomCommon_IFOA(gcbf,''down'')', ...
...
case 'xon',
    ZoomCommon_IFOA(fig,'on') % Set up userprop
    set(fig,'windowbuttondownfcn','ZoomCommon_IFOA(gcbf,''xdown'')', ...
...
case 'yon',
    ZoomCommon_IFOA(fig,'on') % Set up userprop
    set(fig,'windowbuttondownfcn','ZoomCommon_IFOA(gcbf,''ydown'')', ...
        'windowbuttonupfcn','ones;', ...
        'windowbuttonmotionfcn','','buttondownfcn','',...
        'interruptible','on');
    set(ax,'interruptible','on')
...
list = ZoomCommon_IFOA(fig,'getconnect'); % Circular list of connected axes.
if zoomx,
