function varargout=MsgboxGuiIFOA(varargin)
%MSGBOX Message box.
...
if isempty(gcbf)
    ...
    CHandle=guidata(gcbf);
    
    if isfield(CHandle, 'Phandles')
        GCBFOldUnits = get(CHandle.Phandles.figure1,'Units');
        set(CHandle.Phandles.figure1,'Units','points');
        GCBFPos = get(CHandle.Phandles.figure1,'Position');
        set(CHandle.Phandles.figure1,'Units',GCBFOldUnits);
    else        
...
% GcfPos=get(BoxHandle, 'Position');
% 
% set(0,'Units','pixels')
% scnsize = get(0,'ScreenSize');
% 
% set(BoxHandle, 'Position', [round((scnsize(3)-GcfPos(3))/2), round((scnsize(4)-GcfPos(4))/2), GcfPos(3), GcfPos(4)]);
% 
% set(BoxHandle, 'Units', 'characters');
