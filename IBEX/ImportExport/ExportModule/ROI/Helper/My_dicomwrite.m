...
%   $Revision: 1.5.4.7 $  $Date: 2004/12/18 07:35:50 $
...
for p = 1:num_frames
    ...
    if (isequal(lower(options.createmode), 'create'))
        
        [attrs, msg, status] = My_dicom_create_IOD(SOP_UID, X(:,:,:,p), map, ...
                                                metadata, options);
        
    elseif (isequal(lower(options.createmode), 'copy'))
        
        [attrs, msg, status] = My_dicom_copy_IOD(X(:,:,:,p), map, ...
                                              metadata, options);
